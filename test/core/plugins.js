...
describe("Plugins", function () {
    ...
    it("should allow new methods to be defined", function () {
        chakram.addMethod("statusRange", function (respObj, low, high) {
            var inRange = respObj.response.status >= low && respObj.response.status <= high;
            this.assert(
                inRange,
                "expected " + respObj.response.status + " to be between " + low + " and " + high,
                "expected " + respObj.response.status + " not to be between " + low + " and " + high
            );
        });
        ...
        before(function () {
            chakram.addRawPlugin("unavailable", function (chai, utils) {
                utils.addProperty(chai.Assertion.prototype, "unavailable", function () {
                    var status = this._obj.response.status;
                    this.assert(
                        status === 503,
                        "expected status code " + status + " to equal 503",
                        "expected " + status + " to not be equal to 503"
                    );
                });
            });
        });
    ...
    describe("pre 0.2.0 plugin registration", function () {
        before(function () {
            var customProperty = function (chai, utils) {
                utils.addProperty(chai.Assertion.prototype, "teapot", function () {
                    ...
                    this.assert(
                        status === 418,
                        "expected status code " + status + " to equal 418",
                        "expected " + status + " to not be equal to 418"
                    );
                });
            };
            ...
            //         this.assert(
            //             version === ver,
            //             "expected " + version + " to equal #{exp}",
            //             "expected " + version + " to not be equal to #{exp}",
            //             ver
            //         );
            //     });
            // };
            ...
            chakram.initialize(customProperty);
        });
        ...
        // NOTE: disabled test, fetch api does not return httpVersion;
    });
});
