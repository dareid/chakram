...
module.exports = function (chai, utils) {
  ...
  utils.addMethod(chai.Assertion.prototype, 'schema', function () {
    ...
    // filter weird empty string uri
    var missing = tv4.missing.filter(function(s) {
        return s.length > 0;
    });

    var valid = tv4.validate(object, schema) && missing.length === 0;

    var composeErrorMessage = function () {
      var errorMsg = 'expected body to match JSON schema ' + JSON.stringify(schema) + '.';
      if (missing.length > 0) {
        errorMsg += '\n unresolved schemas: ' + JSON.stringify(missing) + '.';
      }
      if(tv4.error !== null) {
